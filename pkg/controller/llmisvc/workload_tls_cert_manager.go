...
func (r *LLMInferenceServiceReconciler) reconcileCertManagerCertificate(ctx context.Context, llmSvc *v1alpha1.LLMInferenceService, config *Config) error {
	log.FromContext(ctx).Info("Reconciling cert-manager Certificate")
	...
	if err := Reconcile(ctx, r, llmSvc, &certmanagerapi.Certificate{}, certificate, semanticCertificateIsEqual); err != nil {
		return fmt.Errorf("failed to reconcile cert-manager certificate: %w", err)
	}

	return r.updateCertManagerCertificateStatus(ctx, llmSvc, config)
}

func (r *LLMInferenceServiceReconciler) expectedCertManagerCertificate(ctx context.Context, llmSvc *v1alpha1.LLMInferenceService, config *Config) (*certmanagerapi.Certificate, error) {
...
func (r *LLMInferenceServiceReconciler) collectIPAddresses(ctx context.Context, svc *v1alpha1.LLMInferenceService) ([]string, error) {
	pods := &corev1.PodList{}
...
func (r *LLMInferenceServiceReconciler) updateCertManagerCertificateStatus(ctx context.Context, llmSvc *v1alpha1.LLMInferenceService, cfg *Config) error {
	expected, err := r.expectedCertManagerCertificate(ctx, llmSvc, cfg)
	if err != nil {
		return fmt.Errorf("failed to get expected certificate: %w", err)
	}

	curr := &certmanagerapi.Certificate{}
	if err := r.Client.Get(ctx, client.ObjectKeyFromObject(expected), curr); err != nil {
		return fmt.Errorf("failed to get cert-manager Certificate %s/%s: %w", expected.GetNamespace(), expected.GetName(), err)
	}

	for _, c := range curr.Status.Conditions {
		if c.Type != certmanagerapi.CertificateConditionReady {
			continue
		}

		if c.Status == certmanagermetaapi.ConditionTrue {
			llmSvc.MarkWorkloadCertificateReady()
			return nil
		}
		llmSvc.MarkWorkloadCertificateNotReady(c.Reason, c.Message)
		return nil
	}

	llmSvc.MarkWorkloadCertificateNotReady("CertificateIssuing", "Issuing Certificate")
	return nil
}
